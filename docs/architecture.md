...
- `src/scry/data/` - feature engineering and the windowing pipeline, plus the data-source seam in `data/sources/`: the `DataSource` interface (`base.py`), the object-store reader (`object_store.py`), and the HttpIngest adapter (`http_ingest.py`).
...
`/predict` takes a window of metrics in the request body and is fully self-contained. `/predict/lookup` pulls a resource's recent metrics through the configured data source first, then predicts.
